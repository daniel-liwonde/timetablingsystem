...
if (isset($_GET['cid'])) {
    ...
    $checker = mysqli_query($conn, "SELECT * FROM checker WHERE courseid='$ccode' ") or die("err1" . mysqli_error($conn));
    $slots = mysqli_fetch_assoc($checker);
    $numofSlots = $slots['slots'];
    if ($numofSlots = 2) {
        mysqli_query($conn, "UPDATE checker SET slots=1 WHERE courseid='$ccode'") or die("err2" . mysqli_error($conn));
    } else {
        mysqli_query($conn, "DELETE FROM checker WHERE courseid='$ccode'") or die("err3" . mysqli_error($conn));
    }
