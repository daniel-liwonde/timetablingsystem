...
if ($sem == 0) {
    ...
    $sql = "SELECT * FROM  wendcourses
        INNER JOIN teacher ON wendcourses.teacher_id=teacher.teacher_id WHERE  wendcourses.allocated!=2";
    ...
    $getCoursesScheduled = mysqli_query($conn, "SELECT * FROM  schedule_wend WHERE sem='$sem'") or die(mysqli_error($conn));
        ...
        while ($room_rows = mysqli_fetch_assoc($rooms)) {
            ...
            for ($i = 0; $i < 6; $i++) { //start time slots
                ...
                if (count($teacherCourses) > 0) { //check if array is valid
                    ...
                    $findCoursesAlreadyScheduled = mysqli_query($conn, "SELECT timeslot FROM  schedule_wend WHERE dayid='$currentDayID' and timeslot='$slot'
				and roomid='$currentRoomID' and sem='$sem'");
                    if (mysqli_num_rows($findCoursesAlreadyScheduled) == 0) { //continue the slot is not filled 
                        ...
                        if (($classClashChecker == false) && ($teacherClashChecker == false)) { //no crash will occour proceed  
                            //schedule course
                            $check = mysqli_query($conn, "SELECT * FROM checker_wend WHERE courseid='$courseid' and sem='$sem'");
...
values('$courseid',1,'$sem')") or die(mysqli_error($conn));
                                    ...
                                    $num = $num + 1;
                                    mysqli_query($conn, "INSERT INTO  schedule_wend(scheduleid,dayid,roomid,timeslot,sem)
 ...
 schedule_wend(scheduleid,dayid,roomid,timeslot,allocatedcourse,lectid,lecturerfname,lecturerlname,dept,sem)
values('$num','$currentDayID','$currentRoomID',$slot,'$course','$teacher_id','$teacherf','$teacherl','$dept','$sem')") or
                                            die(mysqli_error($conn));
                ...
                if (checkRoomCompatibility($conn, $currentRoomID, $courseid) == 1) { //check room capacity
                    ...
                    if (($classClashChecker == false) && ($teacherClashChecker == false)) { //no crash will occour proceed  
                        ...
                        if (mysqli_num_rows($check) == 0) {
                            mysqli_query($conn, "UPDATE  schedule_wend SET allocatedcourse='$course', lectid='$teacher_id',
