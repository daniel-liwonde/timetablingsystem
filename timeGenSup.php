...
$find_weeks = mysqli_query($conn, "SELECT distinct exam_week FROM examschedulesup WHERE sem='$sem'");
if (mysqli_num_rows($find_weeks) == 0) {
    ...
    while ($wrow = mysqli_fetch_assoc($find_weeks)) {
        ...
        <table border="1" class="table table-bordered">
            ...
            $find_dates = mysqli_query($conn, "SELECT distinct edate FROM examschedulesup WHERE exam_week='$eWeek'AND sem='$sem' ORDER BY edate ASC");
                    ...
                    <tr>
                        ...
                        <td colspan="2">
                            <table class="table table-bordered table-hover">
                                <?php
                                $duproom = mysqli_query($conn, "SELECT DISTINCT roomid FROM  examschedulesup WHERE edate='$cdate' AND sessionid='$sessionid' AND sem='$sem'");
                        ...
                        AND exam_week='$eWeek' AND roomid='$roomId'AND sem='$sem'");
                                                while ($row = mysqli_fetch_assoc($getSchedule)) {
                                                    ...
                                                    $rspace = $row['rspace'];
                                                    $dbst = $row['cap'];
                                                    echo "<li>$course($students)($rspace)<a title='remove' onclick='doDeleteSup(" . json_encode($course) . ",$cid)'><i class='fas fa-remove fa-sm'></i></a></li>";
                                                }
