...
if (mysqli_num_rows($sql) == 0) {
    echo json_encode(
        [
            "res" => "<div class='alert alert-danger'> <i class='fas fa-circle-exclamation'></i> &nbsp;Please add supp courses first</div>"
        ]
    );
} else {
                        ...
                        AND exam_week='$i'");
                                    ...
                                    for ($j = 1; $j <= $sessionsPerCourse; $j++) {
                                        if (count($teacherCourses) > 0) { //check if array is valid
                                            ...
                                            if ($foundClash == false) {
                                                //echo "<li> $course</li>";
                                                mysqli_query($conn, "INSERT INTO examschedulesup (edate,courseid,course,sessionid,exam_week) 
