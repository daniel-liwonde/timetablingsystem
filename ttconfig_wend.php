...
if ($sem != 0) {
    ...
    if (isset($_POST['saveRoom'])) {
        ...
        $duproom = mysqli_query($conn, "SELECT rid FROM wendvenues WHERE rid ='$roomName'");
        if (mysqli_num_rows($duproom) == 0) {
            mysqli_query($conn, "INSERT INTO wendvenues(rid)
    ...
    if (isset($_GET['id'])) {
        $del = $_GET['id'];
        $done = mysqli_query($conn, "DELETE FROM wendvenues WHERE rid='$del'");
        if ($done) { //done
    ...
    if (isset($_GET['subid'])) {
        ...
        $done = mysqli_query($conn, "DELETE FROM schedule_wend WHERE timeslot=$tslot and roomid=$room and dayid=$day AND subject_id=$sub AND sem='$sem'");
        if ($done) { //done
            if (mysqli_affected_rows($conn) > 0) {
                $msgP = "<div class='alert alert-success'><i class='fas fa-check-circle'></i> &nbsp; {$subname}  Removed from schedule successifully!</div>";

                $getsub = mysqli_query($conn, "SELECT * FROM wendcourses WHERE subject_id='$sub'");
                $Sessions = mysqli_fetch_assoc($getsub);

                if ($Sessions['allocated'] == 2) {
                    mysqli_query($conn, "UPDATE wendcourses SET allocated=1 WHERE subject_id='$sub' AND sem='$sem'");
                    mysqli_query($conn, "UPDATE checker_wend SET slots=1 WHERE courseid='$sub' AND sem='$sem'");
                } else {
                    mysqli_query($conn, "UPDATE wendcourses SET allocated=0 WHERE subject_id='$sub' AND sem='$sem'");
                    mysqli_query($conn, "DELETE FROM checker_wend  WHERE courseid='$sub' AND sem='$sem'");
                }
            }
        } //close done
    }
                    ...
                    <div class="span12" style="border:1px; width:107%; margin-top:20px; ">
                        ...
                        <div class="hero-unit-3" style="margin-top:10px"> <!--wrapper -->
                            <div class="alert alert-info">
                                ...
                                <strong><i class="icon-calendar icon-large"></i>&nbsp;<span
                                        style="text-decoration:blink">Weekend Teaching Timetable
                                        Settings</span></strong>
                            </div>
                            ...
                            <div class="hero-unit-3" style="margin-top:10px">
                                ...
                                <table cellpadding="0" cellspacing="0" border="0"
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                        $sql = "SELECT * from wendvenues INNER JOIN rooms ON wendvenues.rid=rooms.id";
                                        $find = mysqli_query($conn, $sql);
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-danger"
                                                        href="ttconfig_wend.php?id=<?php echo $rows['rid'] ?>&menu=5">
                            ...
                            <div class="hero-unit-3" style="margin-top:10px; overflow-y:auto ;">
                                ...
                                <div class="alert alert-info">
                                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                                    <strong><i class="fa-list-check icon-large"></i>&nbsp;Set weekend time table
                                        Preferences</strong>
                                ...
                                <div class="control-group" style="float:left; padding-right:5px">
                                    ...
                                    <div class="controls">
                                        <select id="pcourse" required>
                                            ...
                                            $sql = "SELECT *
        FROM teacher
        INNER JOIN wendcourses ON teacher.teacher_id = wendcourses.teacher_id  order by wendcourses.subject_title asc";
                                            $find = mysqli_query($conn, $sql);
                                ...
                                <div class="control-group" style="float:left; padding-right:5px">
                                    ...
                                    <div class="controls">
                                        <select id="pday" required>
                                            ...
                                            <?php
                                            $find = mysqli_query($conn, "SELECT * FROM week_ends");
                                            //f(mysql_affected_rows($find)>0){
                                ...
                                <div class="control-group" style="float:left; padding-right:5px">
                                    ...
                                    <div class="controls">
                                        <select id="proom" required>
                                            ...
                                            <?php
                                            $find = mysqli_query($conn, "SELECT * FROM wendvenues INNER JOIN  rooms ON wendvenues.rid=rooms.id");
                                            //f(mysql_affected_rows($find)>0){
                            ...
                            <div class="hero-unit-3" style="margin-top:10px">
                                ...
                                <table cellpadding="0" cellspacing="0" border="0"
                                    ...
                                    <tbody>
                                        <?php
                                        $find = mysqli_query($conn, "SELECT * FROM wendcourses WHERE allocated != 0");
                                        if (mysqli_num_rows($find) > 0) {
                                            while ($row = mysqli_fetch_assoc($find)) {
                                                $subid = $row['subject_id'];
                                                $subname = $row['subject_title'];
                                                $find2 = mysqli_query($conn, "SELECT * FROM schedule_wend WHERE subject_id='$subid' and sem='$sem'");
                                                ...
                                                while ($rows = mysqli_fetch_assoc($find2)) {
                                                    ...
                                                    if ($dtag != 0) {
                                                        $fiday = mysqli_query($conn, "SELECT * FROM week_ends  WHERE id='$dtag' ");
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><a onclick="return confirm('Are you sure you want to remove this schedule?')"
                                                                ...
                                                                href="ttconfig_wend.php?subid=<?php echo $subid ?>&dayid=<?php echo $dayid ?>&slotid=<?php echo $slotid ?>&roomid=<?php echo $roomid ?>&subject_title=<?php echo $subname ?>&menu=5">
