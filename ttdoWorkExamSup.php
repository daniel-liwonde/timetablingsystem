...
if ($current_date < $today) {
    ...
    if ($current_day_of_week == 6 || $current_day_of_week == 0) {
        ...
        if ($is_holiday) {
            ...
            $findcourse = mysqli_query($conn, "SELECT * FROM suppcourses WHERE subject_id='$courseid'");
            ...
            if (mysqli_num_rows($dup) == 0) {
                $getSchedule = mysqli_query($conn, "SELECT * FROM examschedulesup WHERE edate='$eDate' AND sessionid='$session'");
                ...
                if ($sessionClash == false) { //no crash will occour proceed  
                    mysqli_query($conn, "INSERT INTO examschedulesup (edate,courseid,course,sessionid,exam_week,pref) 
